...
const Sidebar = () => {
    ...
    return (
        <div>
            <div
                className={` ${ShowContext ? "absolute" : "hidden"}  w-[200px]
    bg-white h-screen  top-0 z-40
     shadow-blue-200 shadow-md
    p-5`}>
                <button onClick={() => setShowContext(!ShowContext)} className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" >
                    ✕
                </button>
                <button
        ...
        hover:scale-105 transition-all mt-1">
            ...
            ${pathname?.includes(item.href) ? 'bg-blue-500 text-white'
                                    : null}`}>
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
